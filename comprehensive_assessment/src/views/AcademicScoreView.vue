<template>
  <v-container fluid>
      ...
      <v-col cols="12">
        <v-card>
          <v-card-title class="d-flex align-center">
            <v-icon color="primary" class="me-3">mdi-school-outline</v-icon>
            <span class="text-h5">学业成绩管理</span>
            <v-spacer></v-spacer>
          ...
          <v-card-text>
            ...
            <v-data-table
              ...
              :items-per-page="15"
            >
              <template v-slot:item.academicScore="{ item }">
                <v-text-field
                  v-model.number="item.academicScore"
                  type="number"
                  min="0"
                  max="100"
                  step="0.1"
              ...
              <template v-slot:item.grade="{ item }">
                <v-chip :color="getGradeColor(item.academicScore)" small>
                  {{ getGradeText(item.academicScore) }}
                </v-chip>
              </template>
    ...
    <v-dialog v-model="importDialog" max-width="600px">
      <v-card>
        <v-card-title>批量导入学业成绩</v-card-title>
        <v-card-text>
...
export default {
  ...
  setup() {
    ...
    const headers = [
      ...
      { title: '学业成绩', key: 'academicScore', sortable: true },
    ...
    const filteredStudents = computed(() => {
      ...
      return students.value.filter(student =>
        ...
        student.id.includes(search.value)
      )
    })
    ...
    const saveScore = async (student) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          showSnackbar(`${student.name}的成绩保存成功`, 'success')
        } else {
    ...
    const saveAllScores = async () => {
      ...
      try {
        ...
        if (allSuccessful) {
          ...
          showSnackbar('所有成绩保存成功', 'success')
        } else {
          showSnackbar('部分成绩保存失败', 'error')
        }
      } catch (error) {
        showSnackbar('保存失败', 'error')
      } finally {
    ...
    const parseImportText = (text) => {
      ...
      lines.forEach((line, index) => {
        ...
        if (parts.length !== 2) {
          errors.push(`第${lineNumber}行：格式错误，应为"学号,成绩"`)
          return
        }
        ...
        if (isNaN(score) || score < 0 || score > 100) {
          errors.push(`第${lineNumber}行：成绩"${scoreStr}"无效，应为0-100的数字`)
          return
        }
      ...
      try {
        ...
        if (allSuccessful) {
          ...
          importPreview.value.forEach(item => {
            ...
            if (student) {
              student.academicScore = item.score
            }
          })

          showSnackbar(`成功导入${importPreview.value.length}条成绩记录`, 'success')
