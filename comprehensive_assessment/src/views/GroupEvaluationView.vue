<template>
  <v-container fluid>
      ...
      <v-col cols="12">
        <v-card>
          <v-card-title class="d-flex align-center">
            <v-icon color="primary" class="me-3">mdi-account-group</v-icon>
            <span class="text-h5">综合评价分数管理</span>
            <v-spacer></v-spacer>
          ...
          <v-card-text>
            ...
            <v-data-table
              ...
              :items-per-page="15"
            >
              <template v-slot:item.evaluationScore="{ item }">
                <v-text-field
                  v-model.number="item.evaluationScore"
                  type="number"
                  min="0"
                  max="100"
                  step="0.1"
              ...
              <template v-slot:item.grade="{ item }">
                <v-chip :color="getGradeColor(item.evaluationScore)" small>
                  {{ getGradeText(item.evaluationScore) }}
                </v-chip>
              </template>
    ...
    <v-dialog v-model="importDialog" max-width="600px">
      <v-card>
        <v-card-title>批量导入综合评价分数</v-card-title>
        <v-card-text>
...
const headers = [
  ...
  { title: '综合评价分数', key: 'evaluationScore', sortable: true },
  ...
  try {
    ...
    if (allSuccessful) {
      ...
      importPreview.value.forEach(item => {
        ...
        if (student) {
          student.evaluationScore = item.score;
        }
      });
